...
import { FlopBetting } from "./FlopBetting";

export class DealFlop implements GameState {
  phase = "flop" as const;
  ...
  onEnter(ctx: GameContext): GameContext {
    ...
    const { cards: flop, remaining: afterFlop } = dealCards(afterBurn, 3);
    newCtx.deck = afterFlop;
    newCtx.communityCards = flop;
    ...
    if (eligiblePlayers.length < 2) {
      ...
      const firstActor = getNextEligiblePlayer(
        newCtx.buttonSeat,
        newCtx.players
      );
    ...
    newCtx.currentPhase = "flop";
    newCtx = addToHistory(newCtx, "Flop dealt");

    return newCtx;
  }

  onAction(ctx: GameContext, action: Action): GameContext {
    // No actions allowed during deal
    return ctx;
  }
  ...
  shouldTransition(ctx: GameContext): boolean {
    return true; // Always transition immediately
  }

  getNextState(): GameState | null {
    return new FlopBetting();
  }
}
