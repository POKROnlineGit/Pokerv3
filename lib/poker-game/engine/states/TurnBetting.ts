...
import { DealRiver } from './DealRiver';
import { updatePlayerHandTypes } from '../evaluation/handTypeUpdater';

export class TurnBetting implements GameState {
  phase = 'turn' as const;

  onEnter(ctx: GameContext): GameContext {
    // Update player hand types now that turn is dealt
  ...
  getLegalActions(ctx: GameContext, seat: number): ActionType[] {
    ...
    if (!player || player.folded || player.allIn || !player.eligibleToBet) return [];
