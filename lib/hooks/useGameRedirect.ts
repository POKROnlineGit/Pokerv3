...
export function useGameRedirect() {
  ...
  useEffect(() => {
    ...
    const setupRedirect = async () => {
      ...
      const handleGameRedirect = (game: any, eventType: 'INSERT' | 'UPDATE') => {
        if (!mounted) return

        const gameId = game.id
        const gameStatus = game.status

        console.log(`[GameRedirect] Game ${eventType} event`, { 
          gameId, 
          status: gameStatus,
        ...
        if (gameId.startsWith('local-')) {
          console.log('[GameRedirect] Skipping local game redirect')
          return
        }

        // Check if current user is in the game
        if (!isUserInGame(game, user.id)) {
          console.log('[GameRedirect] User is not a player in this game, ignoring', {
            userId: user.id,
            playerIds: game.player_ids || [],
            players: game.players || []
          })
        ...
        console.log('[GameRedirect] ✅ Game event for me!', { gameId, status: gameStatus, eventType })
        ...
        if (gameStatus === 'starting' || gameStatus === 'active') {
          ...
          if (pathname === `/play/game/${gameId}`) {
            console.log('[GameRedirect] Already on game page, skipping redirect')
            return
          }
          ...
          setTimeout(() => {
            if (mounted) {
              console.log('[GameRedirect] Redirecting to game:', gameId)
              router.push(`/play/game/${gameId}`)
            }
          }, 100)
        } else {
          console.log('[GameRedirect] Game status is not starting/active:', gameStatus)
        }
      }
      ...
      channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (oldStatus !== newStatus && (newStatus === 'starting' || newStatus === 'active')) {
              handleGameRedirect(payload.new, 'UPDATE')
            }
          }
        )
        .subscribe((status) => {
          console.log('[GameRedirect] Subscription status:', status)
        })

      console.log('[GameRedirect] Listening for game creation events for user:', user.id)
    }
