  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      if (data) {
        ...
        console.log(
          "VPIP count type:",
          typeof data.vpip_count,
          "value:",
          data.vpip_count
        );
        console.log(
          "PFR count type:",
          typeof data.pfr_count,
          "value:",
          data.pfr_count
        );
        ...
        const vpipValue =
          data.vpip_count !== undefined
            ? Number(data.vpip_count)
            : data.vpip !== undefined
            ? Number(data.vpip)
            : 0;
        const pfrValue =
          data.pfr_count !== undefined
            ? Number(data.pfr_count)
            : data.pfr !== undefined
            ? Number(data.pfr)
            : 0;
  ...
  useEffect(() => {
    ...
    const fetchHands = async () => {
      const { data, error } = await supabase
        .from("hand_histories")
        .select(
          "id, game_id, hand_index, final_pot, winner_id, played_at, replay_data, player_manifest, config"
        )
    ...
    return (
      <div className="min-h-screen relative">
        <div className="relative z-10">
          <div
            className={cn(
              "container mx-auto py-6 max-w-7xl flex items-center justify-center min-h-screen",
              isMobile ? "px-4" : "px-14"
            )}
          >
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10">
        <div
          className={cn(
            "container mx-auto py-6 max-w-7xl h-[calc(100vh-3rem)] flex flex-col",
            isMobile ? "px-4" : "px-14"
          )}
        >
          <h1
            className={cn("text-3xl font-bold mb-6", isMobile && "text-center")}
          >
            Profile
          </h1>
          {/* Profile Header */}
              ...
              {(() => {
                ...
                const vpip =
                  stats.hands_played > 0
                    ? (stats.vpip_count / stats.hands_played) * 100
                    : 0;
                const pfr =
                  stats.hands_played > 0
                    ? (stats.pfr_count / stats.hands_played) * 100
                    : 0;
                ...
                return (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Card className="bg-card backdrop-blur-sm border">
                      ...
                      <CardContent>
                        <p className="text-3xl font-bold">
                          {stats.hands_played.toLocaleString()} /{" "}
                          {stats.hands_won.toLocaleString()}
                        </p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="bg-card backdrop-blur-sm border">
                      ...
                      <CardContent>
                        <p className="text-3xl font-bold">{vpip.toFixed(1)}%</p>
                      </CardContent>
                    </Card>
