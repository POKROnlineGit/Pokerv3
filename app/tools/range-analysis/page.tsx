...
export default function RangeAnalysisPage() {
  ...
  const loadPreset = (rangeStr: string) => {
    try {
      ...
      const hands = rangeStr.split(',').map(s => s.trim()).filter(s => s.length > 0);
  ...
  useEffect(() => {
    ...
    // Only analyze if valid: 3+ community cards and range is not empty
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 flex items-center min-h-screen">
        <div className="container mx-auto px-6 pt-6 pb-2 max-w-4xl w-full">
          {/* Main Container Box - Single large box containing all three sections */}
          <Card className="p-6 flex flex-col">
            <div className="flex gap-6 flex-1 min-h-0 relative">
                    ...
                    <div className="absolute -bottom-2 left-0 z-10">
                      <div className="w-[180px]">
                        <Select onValueChange={loadPreset}>
                          ...
                          <SelectContent>
                            {presets.map((preset) => (
                              <SelectItem key={preset.id} value={preset.range_string}>
              ...
              {/* RIGHT SIDE: Analysis Results - Full height, extends to right of community cards */}
              <div className="w-64 flex-shrink-0 flex flex-col">
                <Card className="flex-1 flex flex-col min-h-0">
                  <CardHeader className="pb-1 pt-3">
                    <CardTitle className="text-center text-xl font-bold">Range Evaluator</CardTitle>
                  </CardHeader>
                  <CardContent className="pt-3 pb-3 flex-1 flex flex-col">
                    {result ? (
                      <div className="space-y-2">
                        <div>
                          <div className="text-xs text-muted-foreground">Total Valid Combos</div>
                          <div className="text-xl font-bold">{result.validCombos}</div>
                        </div>

                        <div className="pt-2 border-t">
                          <div className="text-xs font-semibold mb-1">Hand Type Distribution</div>
                          {result.stats.length === 0 ? (
                            <p className="text-xs text-muted-foreground">No hand types found</p>
                          ) : (
                      ...
                      <p className="text-xs text-muted-foreground text-center pt-1">
                        {selectedHands.size === 0 || boardCards.length < 3
                          ? "Select hands and 3+ community cards to see results"
                          : "Analyzing..."}
                      </p>
                    )}
                  </CardContent>
